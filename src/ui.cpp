...
static void key_handler(lv_event_t *e) {
    ...
    switch (g_cur) {
        ...
        case SCR_PET:
            if (pet_key(key) == PET_KEY_TO_MENU) request_screen(SCR_MENU);
            break;
...
// Hoat hinh Vector -> uy quyen cho module pet-tricks (chi khi dang o man Pet)
void ui_fast_tick() {
    if (g_cur == SCR_PET) pet_tick(millis());
}
